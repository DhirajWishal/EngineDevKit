...
namespace EDK
{
	namespace Graphics
	{
		namespace VulkanBackend
		{
			...
			{
#if defined(EDK_DEBUG) || defined(EDK_RELEASE)
			...
			void VulkanDevice::CreateSurface()
			{
				if (vSurface == VK_NULL_HANDLE)
					EDK_VK_ASSERT(glfwCreateWindowSurface(GetParentInstance()->Inherit<VulkanBackendInstance>()->GetInstance(), GetWindowHandle(), nullptr, &vSurface), "Failed to create the Vulkan Surface!");
			}
			...
			void VulkanDevice::InitializeLogicalDevice()
			{
				...
				EDK_VK_ASSERT(vkCreateDevice(GetPhysicalDevice(), &createInfo, nullptr, &vLogicalDevice), "Failed to create logical device!");
